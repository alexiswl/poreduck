...
def get_args():
    ...
    parser.add_argument("--overwrite", action='store_true', default=False, help="Overwrite output file rather than append to it")
...
def tar_up_folder(folder_path, output_path, overwrite=False, inplace=False, dry_run=False):
    ...
    if overwrite:
        ...
        # Can't actually append a compressed tar yet We're going to sys.exit  if the file exist
        if os.path.isfile(output_path):
            sys.exit("Tar exists, not overwriting")
...
def get_md5sum(output_path):
    ...
    # Return the md5 of the file for writing to a checksums file
    return md5_output
...
def main():
    ...
    # Tar up folder
    tar_up_folder(args.folder_path, output_path, args.overwrite, args.inplace, args.dry_run)
    # Get md5
    if not args.dry_run:
