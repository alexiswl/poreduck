...
def get_all_files(sequencing_summary_dir, fastq_dir, fast5_dir):
    ...
    fast5_df = pd.DataFrame(fast5_dirs, columns="fast5_dirs")

    # Append number onto each dataframe
    sequencing_summary_df['number'] = sequencing_summary_df['sequencing_summary_file'].apply(
        lambda x: re.match("sequencing_summary_(\d+).txt", x).group(1))
    fastq_df['number'] = fastq_df['fastq_file'].apply(
        lambda x: re.match("fastq_(\d+).fastq", x).group(1))
    fast5_df['number'] = fast5_df['fastq_file'].apply(
        lambda x: re.match('(\d+)', x).group(1))
    ...
    return pd.concat(sequencing_summary_df, fastq_df, fast5_df)


def output_yaml(yaml_file, dataset):
    with open(yaml_file, 'w') as file:
        yaml.dump(json.loads(dataset.to_json()), file, default_flow_style=True)
...
def main():
    ...
    output_yaml(args.yaml_file, dataset)
